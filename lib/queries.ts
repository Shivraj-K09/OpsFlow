...
export const queryKeys = {
  ...
  activity: (workspaceId: string, page: number) => ["activity", workspaceId, page] as const,
...
export function useActivityLogs(workspaceId: string, page: number = 1) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/activity?workspaceId=${workspaceId}&page=${page}`);
...
export function useDashboardMetrics(workspaceId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/metrics/dashboard?workspaceId=${workspaceId}`);
...
export function useWorkloadMetrics(workspaceId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/metrics/workload?workspaceId=${workspaceId}`);
