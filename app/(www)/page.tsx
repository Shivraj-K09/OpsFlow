    ...
    <div className="min-h-screen bg-background text-foreground font-sans flex flex-col relative overflow-hidden">
      ...
      <header className="flex items-center justify-between px-6 sm:px-8 py-5 w-full max-w-7xl mx-auto">
        <div className="flex items-center gap-2 text-primary">
          ...
          <span className="font-semibold text-lg tracking-tight text-foreground">OpsFlow</span>
        </div>
        
        <div className="flex items-center gap-1 sm:gap-2">
          <Button asChild variant="ghost" className="rounded-full h-9 px-5 font-medium">
            <Link href="/login">Log in</Link>
          </Button>
          <Button asChild className="rounded-full h-9 px-5 font-medium shadow-none">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 text-center max-w-4xl mx-auto w-full pb-32 pt-10">
        ...
        <p className="text-base md:text-lg text-muted-foreground mb-10 max-w-2xl mx-auto leading-relaxed">
          The smart task management platform for operations teams. Distribute tasks intelligently and keep your team running at peak efficiency.
        </p>
        
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <Button asChild className="gap-2 shadow-none h-10 px-6 rounded-full font-medium">
            <Link href="/signup">
              Get Started <ArrowRightIcon className="size-4" />
            </Link>
          </Button>
          <Button asChild variant="outline" className="shadow-none h-10 px-6 rounded-full font-medium">
